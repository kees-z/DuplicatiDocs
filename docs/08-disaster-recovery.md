...
How to restore files to the original location of the same system and how to restore files from a consistent backup to a new computer is described in [Restoring files from a backup](#_Restoring_files_from) and [Restoring files if your Duplicati installation is lost](03-using-the-graphical-user-interface/#restoring-files-from-a-backup).
